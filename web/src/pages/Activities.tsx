...
export default function Activities() {
  ...
  function activitySearcher(value: string) {
    ...
    const filteredActivities = patient.activities.filter(activity => activity.name.toLowerCase().includes(value.toLowerCase()));
  ...
  async function removeActivity(activityId: number) {
    ...
    try {
      ...
      const newActivities = patient.activities.filter(activity => activity.id !== activityId);
      ...
      <main className="max-w-5xl mx-auto w-full flex-1 my-8 px-6 sm:px-0">
        <div className="flex flex-col sm:flex-row sm:items-center gap-6 h-fit">
          ...
          <img
            ...
            className="w-40 h-40 max-w-[10rem] max-h-40 rounded-full shadow-image"
          ...
          <div>
            ...
            <span className="text-sm text-description">
              Desde {patient?.createdAt.toLocaleString("pt-BR", { dateStyle: "long" })}
            </span>
          </div>
        </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-end justify-between border-b border-line">
                      ...
                      <div className="flex gap-2 items-end">
                        <HoverCard.Root>
                          ...
                          <span className="text-sm text-[#AAA]">
                            Indicada por <HoverCard.Trigger className="underline cursor-pointer">{activity.author.name}</HoverCard.Trigger>
                          </span>
                        </HoverCard.Root>
